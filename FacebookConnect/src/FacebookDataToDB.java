...
public class FacebookDataToDB extends HttpServlet {
	...
	private void initDB() {
		...
		try {
			...
			connect = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3308/magnumdb?" + "user=ankit&password=ankit");
			System.out.println("Connected to DB");
			// statements allow to issue SQL queries to the database
			statement = (Statement) connect.createStatement();
			// resultSet gets the result of the SQL query
			int result = statement.executeUpdate("insert into user values('346', 'Iron man', 'ironman@buffalo.edu', 'kool', null, null)");
			System.out.println("Result: " + result);
//			ResultSet resultSet = writeResultSet(resultSet);
